...
public static class BlazeUtils
{
    ...
    private static ComputeShader _imageTransformShader;
    private static int s_ImageSample;
    ...
    {
        if (_imageTransformShader == null)
        {
            _imageTransformShader = Resources.Load<ComputeShader>("Shaders/ImageTransform");
        }
        
        int kernel = _imageTransformShader.FindKernel("ImageTransform");

        var tensorData = ComputeTensorData.Pin(dstTensor, false);
    
        _imageTransformShader.SetTexture(kernel, s_X_tex2D, srcTexture);
        _imageTransformShader.SetBuffer(kernel, s_Optr, tensorData.buffer);

        _imageTransformShader.SetInt(s_O_height, dstTensor.shape[1]);
        _imageTransformShader.SetInt(s_O_width, dstTensor.shape[2]);
        _imageTransformShader.SetInt(s_O_channels, dstTensor.shape[3]);
        _imageTransformShader.SetInt(s_X_height, srcTexture.height);
        _imageTransformShader.SetInt(s_X_width, srcTexture.width);

        Matrix4x4 matrix = Matrix4x4.identity;
        matrix.SetColumn(0, new Vector4(M[0][0], M[0][1], 0, 0));
        matrix.SetColumn(1, new Vector4(M[1][0], M[1][1], 0, 0));
        matrix.SetColumn(2, new Vector4(M[2][0], M[2][1], 1, 0)); 
    
        _imageTransformShader.SetMatrix(s_affineMatrix, matrix);

        _imageTransformShader.Dispatch(kernel, IDivC(dstTensor.shape[1], 8), IDivC(dstTensor.shape[1], 8), 1);
    }
